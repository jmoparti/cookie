# -*- coding: utf-8 -*-
"""
Test vagrant 'sys_test_vm' in the emitted template
"""
...
# pytest marker
pytestmark = [pytest.mark.custom_vagrant_test, pytest.mark.custom_sys_test_vm_test]
...
def pytest_generate_tests(metafunc):
    """Metafunc hook for generating parameters for vagrant tests"""
    ...
    if metafunc.function.__name__ == 'test_create_vm':
        vagrant_vms = []
        for vagrant_vm in iter(VagrantVM):
            if vagrant_vm.vm_role == VagrantVMRole.SYSTEM_TEST:
...
def test_create_vm(basic_test_params, specific_test_params):
    func_params = locals()
    ...
    if vagrant_vm.vm_role != VagrantVMRole.SYSTEM_TEST:
        pytest.skip("Vagrant VM {} not in role {}".format(vagrant_vm.vm_name, VagrantVMRole.SYSTEM_TEST))
